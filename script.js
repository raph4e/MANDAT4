...
const LoadImage = async () => { // Fonction asynchrone pour charger des images depuis Pexels
    try {
        const response = await fetch("https://api.pexels.com/v1/curated?per_page=10", { // Requête à l'API Pexels pour obtenir des images (await sert à attendre la réponse de fetch (servant à faire des requêtes HTTP)
